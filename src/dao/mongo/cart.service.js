...
class CartServiceManager {
  ...
  async addProductToCart(idCart, idProduct) {
    try {
      ...
      const productExist = await cartsModel
        .find({
          _id: idCart,
          products: {
      ...
      if (!productExist.length) {
        const productAdd = await cartsModel
          .findOneAndUpdate(
            { _id: idCart },
        ...
        const productAddQuant = await cartsModel
          .findOneAndUpdate(
  ...
  async deleteProductCart(idCart, idProduct) {
    try {
      ...
      const deleteProduct = await cartsModel
        .updateOne(
          { _id: idCart },
          {
            $pull: {
  ...
  async deleteAllCartProducts(idCart) {
    try {
      ...
      const deleteCartProducts = await cartsModel
        .findOneAndUpdate(
          { _id: idCart },
          {
            $set: {
  ...
  async buyCart(user) {
    ...
    const validStockProducts = cartProducts.filter(
      (p) => p.quantity <= p.product.stock
    );
    const invalidStockProducts = cartProducts.filter(
      (p) => p.quantity > p.product.stock
    );
    ...
    const ticket = await ticketModel
      ...
      .then((res) => {
        //remover stock
        validStockProducts.forEach((p) =>
          this.updateProductStock(p.product._id, p.product.stock - p.quantity)
        );
